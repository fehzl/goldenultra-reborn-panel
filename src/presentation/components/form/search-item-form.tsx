...
import { DeviceModel, OrderItemModel } from '@/domain/models';
...
export function SearchItemForm({ items, setItems, selectInputRef }: Props) {
  ...
  const chooseItem = (device: DeviceModel): void => {
    ...
    const item: Omit<
      OrderItemModel,
      'id' | 'discount' | 'overall' | 'amount_separated'
    > = {
      device,
      amount: null,
